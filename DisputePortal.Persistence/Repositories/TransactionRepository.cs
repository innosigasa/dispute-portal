...
public class TransactionRepository(DisputePortalDbContext ctx) : ITransactionRepository
{
        ...
        Guid customerId, Guid? accountId, TransactionFilters filters, int page, int pageSize, CancellationToken ct)
    {
        ...
        var items = await query
            ...
            .Select(t => new TransactionListItemDto(
                ...
                t.AccountId, t.Account.AccountName, t.Account.AccountType.ToString(), t.Account.AccountNumber
            ))
            .ToListAsync(ct);
    ...
    public async Task<TransactionDetailDto?> GetByIdAsync(Guid id, Guid customerId, CancellationToken ct)
    {
        return await ctx.Transactions
            ...
            .Select(t => new TransactionDetailDto(
                ...
                t.Dispute != null
                    ? new DisputeSummaryDto(t.Dispute.Id, t.Dispute.ReferenceNumber, t.Dispute.Status.ToString(), t.Dispute.SubmittedAt)
                    : null,
                new AccountInfoDto(t.AccountId, t.Account.AccountName, t.Account.AccountType.ToString(), t.Account.AccountNumber)
            ))
