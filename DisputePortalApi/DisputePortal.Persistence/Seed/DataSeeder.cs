...
public static class DataSeeder
{
    ...
    public static async Task SeedAsync(DisputePortalDbContext ctx, IServiceCollection services,
        IConfiguration configuration)
    {
        await SeedUsersAsync(ctx, configuration, services);
    ...
    private static async Task SeedUsersAsync(DisputePortalDbContext ctx, IConfiguration configuration,
        IServiceCollection services)
    {
        ...
        ctx.Customers.AddRange(
            new CustomerModel { Id = AliceId, FullName = "Alice Johnson", Email = "alice@example.com", IdentityNumber = "8501015009087", CreatedAt = DateTime.UtcNow },
            new CustomerModel { Id = BobId,   FullName = "Bob Smith",     Email = "bob@example.com",   IdentityNumber = "7908125012083", CreatedAt = DateTime.UtcNow }
        );

        ctx.Users.AddRange(
            new AppUserModel { Id = Guid.NewGuid(), Email = "alice@example.com", PasswordHash = passwordHasher.Hash("Password1!"), Role = "customer", CustomerId = AliceId, CreatedAt = DateTime.UtcNow },
            new AppUserModel { Id = Guid.NewGuid(), Email = "bob@example.com",   PasswordHash = passwordHasher.Hash("Password1!"), Role = "customer", CustomerId = BobId,   CreatedAt = DateTime.UtcNow },
            new AppUserModel { Id = Guid.NewGuid(), Email = "agent@bank.com",    PasswordHash = passwordHasher.Hash("Password1!"), Role = "agent",    CustomerId = null,    CreatedAt = DateTime.UtcNow }
        );
    }
    ...
    private static async Task SeedBankAccountsAsync(DisputePortalDbContext ctx)
    {
        ...
        ctx.BankAccounts.AddRange(
            // Alice — 3 accounts
            new BankAccountModel { Id = AliceSavingsId, CustomerId = AliceId, AccountNumber = "4001-2345-6789", AccountTypeId = AccountType.Savings.ToId(), AccountName = "Alice's Savings",  Balance = 12_450.00m, Currency = AppConstants.CurrencyZAR, IsDefault = true,  CreatedAt = DateTime.UtcNow },
            new BankAccountModel { Id = AliceChequeId,  CustomerId = AliceId, AccountNumber = "4001-9876-5432", AccountTypeId = AccountType.Cheque.ToId(),  AccountName = "Alice's Cheque",   Balance =  3_200.50m, Currency = AppConstants.CurrencyZAR, IsDefault = false, CreatedAt = DateTime.UtcNow },
            new BankAccountModel { Id = AliceCreditId,  CustomerId = AliceId, AccountNumber = "4001-1111-2222", AccountTypeId = AccountType.Credit.ToId(),  AccountName = "Alice's Credit",   Balance = -1_540.00m, Currency = AppConstants.CurrencyZAR, IsDefault = false, CreatedAt = DateTime.UtcNow },

            // Bob — 2 accounts
            new BankAccountModel { Id = BobCurrentId, CustomerId = BobId, AccountNumber = "4009-8765-4321", AccountTypeId = AccountType.Current.ToId(), AccountName = "Bob's Current",  Balance =  8_775.00m, Currency = AppConstants.CurrencyZAR, IsDefault = true,  CreatedAt = DateTime.UtcNow },
            new BankAccountModel { Id = BobSavingsId,  CustomerId = BobId, AccountNumber = "4009-1234-5678", AccountTypeId = AccountType.Savings.ToId(), AccountName = "Bob's Savings",   Balance = 25_000.00m, Currency = AppConstants.CurrencyZAR, IsDefault = false, CreatedAt = DateTime.UtcNow }
        );
    }
    ...
    private static async Task SeedDisputesAsync(DisputePortalDbContext ctx)
    {
        ...
        var disputes = new List<(TransactionModel tx, DisputeStatus status, DisputeReasonCode reason, string comments)>
        {
        ...
        foreach (var (tx, status, reason, comments) in disputes)
        {
            ...
            ctx.Disputes.Add(new DisputeModel
            {
                Id              = disputeId,
                ReferenceNumber = $"DSP-2026-{seq++:D6}",
                TransactionId   = tx.Id,
                CustomerId      = tx.CustomerId,
                DisputeReasonId          = reason.ToId(),
                Comments        = comments,
                StatusId          = status.ToId(),
